...
- Repository: `https://github.com/EnzymeGroup/enzyme-ai-papers`
- Website: `https://enzymegroup.github.io/enzyme-ai-papers/`
...
```yaml
site_url: https://enzymegroup.github.io/enzyme-ai-papers/
repo_url: https://github.com/EnzymeGroup/enzyme-ai-papers
repo_name: EnzymeGroup/enzyme-ai-papers
```
...
```text
https://enzymegroup.github.io/enzyme-ai-papers/
```
...
作用：这是组织维护者快速通道。它会直接写入 `main`，并在同一个 workflow
里完成 `build_docs`、metadata validation、tests、MkDocs build 和 Pages 部署。

注意：这个 workflow 由仓库变量 `ENZYME_PAPERS_DIRECT_PUBLISHERS` 控制。
默认允许 `cnjxt`，需要多人维护时把变量设为逗号分隔用户名，例如
`cnjxt,another-maintainer`。
...
作用：这是组织维护者纠错通道。它会重新生成 `README.md` 和 `docs/`，跑
validation/tests/MkDocs build，提交到 `main`，并部署 Pages。可运行用户同样由
`ENZYME_PAPERS_DIRECT_PUBLISHERS` 控制。
